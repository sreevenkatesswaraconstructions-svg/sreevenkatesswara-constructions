...
export default function SubmitTestimonialPage() {
  ...
  const projectId = useMemo(() => {
    const value = router.query?.project
  ...
  useEffect(() => {
    ...
    if (!projectId) return
    const storedProject = typeof window !== 'undefined' ? window.sessionStorage.getItem('testimonialProjectId') : null
    if (storedProject !== projectId && typeof window !== 'undefined') {
      window.sessionStorage.setItem('testimonialProjectId', projectId)
    }
  ...
  const uploadPhoto = async () => {
    ...
    try {
      ...
      console.error('[TESTIMONIAL SUBMIT] Photo upload failed', err)
      return null
    }
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error('[TESTIMONIAL SUBMIT] Submission failed', err)
  ...
  return (
    <Layout>
      <SEO title="Submit Your Testimonial" description="Submit your testimonial for Sree Venkatesswara Constructions & Interiors" />
      <main className="mx-auto max-w-3xl px-6 py-12">
        <div className="space-y-6 text-center">
          <p className="text-sm uppercase tracking-[0.35em] text-gold">Share Your Experience</p>
          <h1 className="text-4xl font-serif text-emerald">Leave Your Feedback</h1>
          <p className="mx-auto max-w-2xl text-gray-700">
            Thank you for choosing us. Your feedback helps us improve and helps future customers feel confident about their next project.
          </p>
        </div>

        <div className="mt-10 rounded-[1.5rem] border border-emerald/10 bg-white/80 p-8 shadow-2xl">
          {success ? (
            <div className="py-12 text-center">
              <h2 className="text-2xl font-semibold text-emerald">Thank you for sharing your experience!</h2>
              <p className="mt-4 text-gray-700">Your review has been submitted successfully and is currently under review.</p>
            ...
            <form onSubmit={handleSubmit} className="grid gap-4 md:grid-cols-2">
              ...
              <div>
                ...
                <select value={projectType} onChange={(e) => setProjectType(e.target.value)} className="mt-1 block w-full rounded-md border border-emerald/20 bg-white p-3">
                ...
                <div className="mt-2 flex items-center gap-2">
                  {[1, 2, 3, 4, 5].map((st) => (
                    <button
                      ...
                      className={`rounded-md border px-3 py-2 ${rating >= st ? 'border-emerald bg-emerald text-white' : 'border-emerald/20 text-gray-700'}`}
                    >
                      {st} ★
                    </button>
                  ))}
              ...
              <div className="md:col-span-2 mt-4 flex items-center justify-between">
                ...
                <button type="submit" disabled={loading} className="inline-flex items-center justify-center rounded-full bg-emerald px-6 py-3 font-semibold text-white">
